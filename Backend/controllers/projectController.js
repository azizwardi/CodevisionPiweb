  ...
  try {
    ...
    if (req.user && req.user.role === "admin") {
      return res.status(403).json({
        message:
          "Les administrateurs ne sont pas autorisés à créer des projets",
        isAdmin: true,
      });
    }
...
exports.updateProject = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "admin") {
      return res.status(403).json({
        message:
          "Les administrateurs ne sont pas autorisés à modifier des projets",
        isAdmin: true,
      });
    }
...
exports.deleteProject = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "admin") {
      return res.status(403).json({
        message:
          "Les administrateurs ne sont pas autorisés à supprimer des projets",
        isAdmin: true,
      });
    }
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const users = await User.find(query).select(
      "_id username email firstName lastName role"
    );
...
exports.addMemberToProject = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "admin") {
      return res.status(403).json({
        message:
          "Les administrateurs ne sont pas autorisés à ajouter des membres aux projets",
        isAdmin: true,
      });
    }
...
exports.removeMemberFromProject = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "admin") {
      return res.status(403).json({
        message:
          "Les administrateurs ne sont pas autorisés à supprimer des membres des projets",
        isAdmin: true,
      });
    }
