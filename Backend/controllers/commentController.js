  ...
  try {
    ...
    if (req.user && req.user.role === 'admin') {
      return res.status(403).json({
        message: "Les administrateurs ne sont pas autorisés à créer des commentaires",
        isAdmin: true
      });
    }
...
exports.updateComment = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === 'admin') {
      return res.status(403).json({
        message: "Les administrateurs ne sont pas autorisés à modifier des commentaires",
        isAdmin: true
      });
    }
...
exports.deleteComment = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === 'admin') {
      return res.status(403).json({
        message: "Les administrateurs ne sont pas autorisés à supprimer des commentaires",
        isAdmin: true
      });
    }
