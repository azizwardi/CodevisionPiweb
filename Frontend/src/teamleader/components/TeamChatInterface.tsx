...
const TeamChatInterface: React.FC<TeamChatInterfaceProps> = ({ teamId, userId, team }) => {
  ...
  useEffect(() => {
    console.log('TeamLeader - Setting up socket connection for team:', teamId);
    ...
    try {
      ...
      newSocket.on('connect', () => {
        console.log('TeamLeader - Socket.IO connected for team chat with socket ID:', newSocket.id);
        // Join the team room
        newSocket.emit('joinTeam', teamId);
        console.log('TeamLeader - Emitted joinTeam event for team:', teamId);
      ...
      newSocket.on('connect_error', (error) => {
        console.error('TeamLeader - Socket.IO connection error:', error);
      });
      ...
      newSocket.on('newTeamMessage', (message: any) => {
        ...
        setMessages(prevMessages => {
          ...
          if (prevMessages.some(m => m._id === message._id)) {
            console.log('TeamLeader - Message already exists in state');
            return prevMessages;
          }
      ...
      newSocket.on('messageSent', (data: any) => {
        console.log('TeamLeader - Message sent confirmation received:', data);

        if (data.tempMessageId) {
          // The temporary message was successfully sent and saved
          console.log('TeamLeader - Temporary message was successfully sent:', data.tempMessageId);
        }
      });

      // Handle message error
      newSocket.on('messageError', (data: any) => {
        console.error('TeamLeader - Message error received:', data);
      ...
      newSocket.on('disconnect', () => {
        console.log('TeamLeader - Socket.IO disconnected from team chat');
      });

      setSocket(newSocket);

      return () => {
        console.log('TeamLeader - Cleaning up socket connection for team:', teamId);
      ...
      console.error('TeamLeader - Error setting up socket connection:', error);
    }
  }, [teamId, userId]);
  ...
  useEffect(() => {
    if (teamId) {
      console.log('TeamLeader - Team ID changed, fetching messages for team:', teamId);
      console.log('TeamLeader - Current user ID:', userId);
      fetchMessages();
    }
  }, [teamId, userId]);
  ...
  const fetchMessages = async () => {
    try {
      setLoading(true);
      setError('');
      console.log('TeamLeader - Fetching messages from API for team:', teamId);

      try {
        const response = await axios.get(`http://localhost:5000/team-chat/${teamId}/messages`);
        console.log('TeamLeader - Messages API response status:', response.status);
        console.log('TeamLeader - Messages API response data:', response.data);

        if (response.data && Array.isArray(response.data.messages) && response.data.messages.length > 0) {
          console.log(`TeamLeader - Loaded ${response.data.messages.length} messages from server`);
          ...
          console.log('TeamLeader - No messages found from API');

          // Try to send a welcome message to initialize the chat
          const welcomeMessage: Message = {
            _id: `welcome-${Date.now()}`,
            team: teamId,
            sender: {
              _id: userId,
              username: 'You', // Changed to 'You' for consistent display
              email: '',
              firstName: '',
              lastName: ''
            },
            content: 'Welcome to the team chat! This is where we can communicate and collaborate as a team.',
            createdAt: new Date().toISOString(),
            readBy: [userId]
          };

          console.log('TeamLeader - Sending welcome message to server');
          ...
          try {
            ...
            console.log('TeamLeader - Welcome message sent successfully:', sendResponse.data);
            ...
            console.error('TeamLeader - Error sending welcome message:', sendError);
            setMessages([welcomeMessage]);
          }
        }
      } catch (apiError) {
        console.error('TeamLeader - API error fetching messages:', apiError);
        setError('Failed to load messages. Please try again later.');
        toastManager.addToast('Failed to load messages', 'error', 5000);
      }
    } catch (error) {
      console.error('TeamLeader - Error in fetchMessages function:', error);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      try {
        console.log('TeamLeader - Sending message to server');
        ...
        console.log('TeamLeader - Message sent successfully, response:', response.data);
        ...
        console.error('TeamLeader - Error sending message:', error);
        toastManager.addToast('Failed to send message', 'error', 5000);
      }
    } catch (error) {
      console.error('TeamLeader - Error in sendMessage function:', error);
      toastManager.addToast('Failed to send message', 'error', 5000);
    } finally {
      setLoading(false);
    }
  ...
  const scrollToBottom = () => {
    if (messagesEndRef.current) {
      console.log('TeamLeader - Scrolling to bottom of messages');
      messagesEndRef.current.scrollIntoView({ behavior: 'smooth' });
    } else {
      console.log('TeamLeader - messagesEndRef is not available for scrolling');
    }
  };
